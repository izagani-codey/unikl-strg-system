                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border-l-4 {{ $borderColor }}">
                            ...
                            @if($stepStatus === 'completed' && $step['id'] === 'verified' && $request->verifiedBy)
                                <div class="text-xs text-gray-500 mt-2">
                                    ...
                                    @if($request->auditLogs()->where('to_status', \App\Enums\RequestStatus::STAFF1_APPROVED->value)->first())
                                        <span class="ml-2">on {{ $request->auditLogs()->where('to_status', \App\Enums\RequestStatus::STAFF1_APPROVED->value)->first()->created_at->format('d M Y, h:i A') }}</span>
                                    @endif
                                </div>
                            @elseif($stepStatus === 'completed' && $step['id'] === 'recommended' && $request->recommendedBy)
                                <div class="text-xs text-gray-500 mt-2">
                                    ...
                                    @if($request->auditLogs()->where('to_status', \App\Enums\RequestStatus::STAFF2_APPROVED->value)->first())
                                        <span class="ml-2">on {{ $request->auditLogs()->where('to_status', \App\Enums\RequestStatus::STAFF2_APPROVED->value)->first()->created_at->format('d M Y, h:i A') }}</span>
                                    @endif
                                </div>
                            ...
                            @if($request->status_id === \App\Enums\RequestStatus::REJECTED->value && $step['id'] === 'recommended')
