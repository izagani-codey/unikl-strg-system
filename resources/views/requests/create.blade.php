<x-app-layout>
    ...
    <script>
        ...
        function initializeVotItems() {
            const normalizedOldItems = Array.isArray(oldVotItems)
                ? oldVotItems
                : Object.values(oldVotItems || {});

            if (normalizedOldItems.length > 0) {
                normalizedOldItems.forEach((item) => addVotItemRow(item));
                return;
            }
            addVotItemRow();
        }
        ...
        function addVotItemRow() {
            ...
            select.name = `vot_items[${votItemCount}][vot_code]`;
            descInput.name = `vot_items[${votItemCount}][description]`;
            amountInput.name = `vot_items[${votItemCount}][amount]`;
            descInput.required = false;

            if (initialData && initialData.vot_code) {
                select.value = initialData.vot_code;
                const selectedOption = select.options[select.selectedIndex];
                const description = (selectedOption && selectedOption.dataset ? selectedOption.dataset.description : '') || initialData.description || '';
                descInput.value = description;
                row.querySelector('.vot-desc-preview').textContent = description ? `Description: ${description}` : '';
            }

            if (initialData && initialData.amount !== undefined && initialData.amount !== null) {
